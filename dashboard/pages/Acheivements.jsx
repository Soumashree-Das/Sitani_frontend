    ...
    {showForm && (
      <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
        <div className="bg-[#FBFFF1] rounded-lg p-4 sm:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-stone-700">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-stone-700 mb-1 text-sm sm:text-base">Title*</label>
              <input
                type="text"
                value={formData.title}
                onChange={(e) =>
            ...
            <div>
              <label className="block text-stone-700 mb-1 text-sm sm:text-base">Description</label>
              <textarea
                value={formData.description}
                onChange={(e) =>
                  setFormData({ ...formData, description: e.target.value })
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-stone-700 mb-1 text-sm sm:text-base">Category*</label>
                <select
                  value={formData.category}
                  onChange={(e) =>
                    setFormData({ ...formData, category: e.target.value })
            ...
            <div className="flex items-start gap-3">
              ...
              />
              <label htmlFor="featured" className="text-stone-700 text-sm sm:text-base leading-relaxed">
                Featured Acheivement (featured acheivements will be displayed on the website)
              </label>
            </div>
    ...
    <div className="grid gap-3 sm:gap-4">
      {acheivements.length === 0 ? (
        <div className="text-center py-8 sm:py-12 text-stone-600">
          <p className="text-sm sm:text-base">No acheivements found. Add your first acheivement!</p>
        </div>
      ) : (
        acheivements.map((acheivement) => (
          <div
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 text-xs sm:text-sm">
                  ...
                  <span className="text-stone-500 py-1">
                    {acheivement.dateAchieved 
                      ? new Date(acheivement.dateAchieved).toLocaleDateString()
                      : 'No date'}
                  </span>
                </div>
                {acheivement.tags && acheivement.tags.length > 0 && (
